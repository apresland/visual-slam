...
void Tracker::track(std::shared_ptr<Frame> frame) {
    ...
    std::vector<cv::Point2f> frame_t0_points_left_ = frame->get_points_left();
    std::vector<cv::Point2f> frame_t0_points_right_ = frame->get_points_left();
    ...
    calcOpticalFlowPyrLK(frame->image_left_, frame->image_right_, frame_t0_points_left_, frame_t0_points_left_, status, err, window_size, 3, term_criteria, 0, 0.001);
    ...
    for(int i=0; i < status.size(); i++)
    {
        ...
        cv::Point2f point = frame_t0_points_right_.at(i - deletion_correction);
    ...
    frame->set_features_left(frame_t0_points_left_);
    frame->set_features_right(frame_t0_points_right_);
}
